...
static void synaptics_secure_touch_init(struct synaptics_rmi4_data *data)
{
	...
	/* Get clocks */
	data->core_clk = devm_clk_get(&data->i2c_client->dev, "core_clk");
	if (IS_ERR(data->core_clk)) {
		input_err(true, &data->i2c_client->dev,
			"%s: error on clk_get(core_clk): %ld\n", __func__,
			PTR_ERR(data->core_clk));
		data->core_clk = NULL;
	}

	data->iface_clk = devm_clk_get(&data->i2c_client->dev, "iface_clk");
	if (IS_ERR(data->iface_clk)) {
		input_err(ture, &data->i2c_client->dev,
				    ...
				    const char *buf, size_t count)
{
	...
	case 0:
		...
		synaptics_rmi4_i2c_put(rmi4_data);
	...
	case 1:
		...
		if (synaptics_rmi4_i2c_get(rmi4_data) < 0) {
			input_err(treu,
...
{
	int retval;
	struct i2c_client *i2c = to_i2c_client(&rmi4_data->i2c_client->dev);
	...
	if (retval >= 0 && rmi4_data->core_clk != NULL &&
		...
		if (retval)
			pm_runtime_put_sync(&rmi4_data->i2c_client->dev);
	}
	mutex_unlock(&rmi4_data->rmi4_io_ctrl_mutex);
...
static void	synaptics_init_product_info_v7(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (strncmp(rmi4_data->rmi4_mod_info.product_id_string, "TD4100", 6) == 0) {
...
#endif
	struct synaptics_rmi4_fn *temp_fhandler;
	f01found = false;
	f35found = false;
...
static int synaptics_parse_dt(struct i2c_client *client)
{
	...
	pdata->bringup = of_property_read_bool(np, "synaptics,bringup");
...
#if defined(CONFIG_SECURE_TOUCH)
	rmi4_data->i2c_get = synaptics_rmi4_i2c_get;
	rmi4_data->i2c_put = synaptics_rmi4_i2c_put;
#endif
...
#if defined(CONFIG_SECURE_TOUCH)
	for (attr_count = 0; attr_count < ARRAY_SIZE(secure_attrs); attr_count++) {
		retval = sysfs_create_file(&rmi4_data->input_dev->dev.kobj,
				&secure_attrs[attr_count].attr);
		if (retval < 0) {
			input_err(true, &rmi4_data->i2c_client->dev,
					"%s: Failed to create secure sysfs attributes\n",
					__func__);
			goto err_secure_sysfs;
		}
	}
#endif
...
*/
	...
	if (IS_ERR(pinctrl_state)) {
		input_err(true, &rmi4_data->i2c_client->dev, 
			"%s: Failed to lookup pinctrl.\n", __func__);
	} else {
		retval = pinctrl_select_state(rmi4_data->pinctrl, pinctrl_state);
		if (retval)
			input_err(true, &rmi4_data->i2c_client->dev,
				"%s: Failed to configure pinctrl.\n", __func__);
	}
