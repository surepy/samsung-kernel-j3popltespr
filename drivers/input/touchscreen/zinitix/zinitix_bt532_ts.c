...
static bool ts_read_coord(struct bt532_ts_info *info)
{
	...
	if((info->pdata->support_lpm_mode)&&(zinitix_bit_test(info->touch_info.status, BIT_GESTURE))){
		if (read_data(info->client, ZT75XX_LPM_MODE_REG, (u8 *)&lpm_mode_reg, 2) < 0)
			tsp_debug_info(true, &client->dev, "%s: lpm_mode_reg read fail\n", __func__);
		tsp_debug_info(true, &client->dev, "lpm_mode_reg read  0x%02x%02x \n", lpm_mode_reg.data, lpm_mode_reg.flag);
		...
		unsigned long cmd, void *data)
{
	...
	default:
		...
		if (atomic_read(&misc_info->secure_enabled)) {
			tsp_debug_info(true, &info->client->dev, "%s: ignored, because secure mode, old:%d, TA:%d\n", __func__, old_ta_status, g_ta_connected);
			return 0;
		}else{
...
static bool ts_hw_calibration(struct bt532_ts_info *info)
{
	...
	if(ret < 0){
		tsp_debug_info(true, &client->dev, "%s : data read fail\n", __func__);
		return false;
	}
	tsp_debug_info(true, &info->client->dev, "%s : %02x %02x %02x %02x %02x %02x", 
	...
	tsp_debug_info(true, &client->dev, "%s: pat_function=%d afe_base=%04X cal_count=%X tune_fix_ver=%04X\n", 
	...
	if(info->cap_info.cal_count == 0xff){
		...
		tsp_debug_info(true, &client->dev, "%s: initialize nv as default value & excute autotune \n", __func__);
	...
	if(info->pdata->pat_function == PAT_CONTROL_NONE && info->cap_info.cal_count != 0){
		tsp_debug_info(true, &client->dev, "%s : skip\n", __func__);
		return true;
	}
	/* pat_function(2) */
...
start_calibration:
	...
	tsp_debug_info(true, &client->dev, "%s end\n", __func__);
	tsp_debug_info(true, &client->dev, "%s: pat_function=%d cal_count=%X tune_fix_ver=%02X%02X\n", 
...
static int fw_update_work(struct bt532_ts_info *info, bool force_update)
{
	...
	if(pdata->bringup){
		tsp_debug_info(true, &info->client->dev, "%s: bringup\n", __func__);
		return 0;
	}
...
static void bt532_ts_close(struct input_dev *dev)
{
	...
	if((pdata->support_lpm_mode)&&(info->spay_enable||info->aod_enable)){
		...
		if (write_reg(info->client, ZT75XX_LPM_MODE_REG, lpm_mode_reg.flag) != I2C_SUCCESS)
			tsp_debug_info(true, &misc_info->client->dev, "%s, fail lpm mode set\n", __func__);
...
static void run_selfdnd_read(void *device_data)
{
	...
	tsp_debug_info(true,&client->dev, "SELF DND start\n");
	...
	tsp_debug_info(true, &client->dev, "SELF DND Pass\n");
...
static void run_selfdnd_h_gap_read(void *device_data)
{
	...
	tsp_debug_info(true, &client->dev, "SELFDND H Gap screen_max %d\n", screen_max);
...
static void run_tsp_rawdata_read(void *device_data, u16 rawdata_mode, s16* buff)
{
	...
	tsp_debug_info(true, &info->client->dev, "touch rawdata %d start\n", rawdata_mode);
...
const s16 DEF_MIS_CAL_SPEC_MAX[TSP_CMD_NODE_NUM] =
{
...
const s16 DEF_MIS_CAL_SPEC_MIN[TSP_CMD_NODE_NUM] =
{
...
static void run_mis_cal_read(void * device_data)
{
	...
	if (pdata->mis_cal_check == 0) {
		tsp_debug_info(true, &info->client->dev, "%s: [ERROR] not support\n", __func__);
		mis_cal_data = 0xF1;
		goto NG;
	}
	...
	if (read_data(info->client, BT532_EEPROM_INFO, (u8 *)&chip_eeprom_info, 2) < 0){
		tsp_debug_info(true, &info->client->dev, "%s: read eeprom_info i2c fail!\n", __func__);
		mis_cal_data = 0xF3;
		goto NG;
	}

	if (zinitix_bit_test(chip_eeprom_info, 0)){
		tsp_debug_info(true, &info->client->dev, "%s: eeprom cal 0, skip !\n", __func__);
		mis_cal_data = 0xF1;
		goto NG;
	} 
	...
	if (!ret) {
		tsp_debug_info(true, &info->client->dev, "%s:[ERROR] i2c fail!\n", __func__);
		ts_set_touchmode(TOUCH_POINT_MODE);
		mis_cal_data = 0xF4;
		goto NG;
	}
	...
	for (i = 0; i < x_num; i++) {
		for (j = 0; j < y_num; j++) {
			...
			if(ret 
				&&(raw_data->reference_data_abnormal[offset] > DEF_MIS_CAL_SPEC_MAX[offset]
				|| raw_data->reference_data_abnormal[offset] < DEF_MIS_CAL_SPEC_MIN[offset])){
...
static void get_pat_information(void *device_data)
{
	...
	tsp_debug_info(true, &info->client->dev, "%s : P%02XT%04X", 
...
static void get_tsp_test_result(void *device_data)
{
	...
	tsp_debug_info(true, &info->client->dev, "%s : %X", __func__, info->test_result.data[0]);

	if (info->test_result.data[0] == 0xFF) {
		tsp_debug_info(true, &info->client->dev, "%s: clear factory_result as zero\n", __func__);
...
void set_tsp_nvm_data(struct bt532_ts_info *info, u8 addr, u8 *values, u16 length)
{
	...
	esd_timer_start(CHECK_ESD_TIMER, misc_info);
#endif
	tsp_debug_info(true, &client->dev, "%s end", __func__);
	return;
...
static void ium_random_write(struct bt532_ts_info *info, u8 data)
{
	...
	tsp_debug_info(true, &client->dev, "%s %x %d", __func__, data, data);
	...
	esd_timer_start(CHECK_ESD_TIMER, misc_info);
#endif
	tsp_debug_info(true, &client->dev, "%s %d", __func__, __LINE__);
	return;
...
static void ium_random_read(struct bt532_ts_info *info, u8 data)
{
	...
	tsp_debug_info(true, &info->client->dev, "%s %x %d", __func__, buff[data], buff[data]);
...
static void ium_r_write(void *device_data)
{
	...
	tsp_debug_info(true, &info->client->dev, "%s %x %x", __func__, finfo->cmd_param[0], finfo->cmd_param[1]);
...
static void ium_read(void *device_data)
{
	...
	for( i=63 ; i>0 ; i-=4) {
		tsp_debug_info(true, &client->dev, "%s: [%2d]:%02X %02X %02X %02X\n",
...
static void run_ref_calibration(void *device_data)
{
	...
	if(info->finger_cnt1 != 0){
		tsp_debug_info(true, &client->dev, "%s: return (finger cnt %d)\n", __func__, info->finger_cnt1);
		snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", "NG");
		set_cmd_result(info, finfo->cmd_buff, (int)strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
		finfo->cmd_state = FAIL;
		return;
...
static void set_aod_rect(void *device_data)
{
	...
	tsp_debug_info(true, &info->client->dev, "%s: w:%d, h:%d, x:%d, y:%d\n",
			 ...
			 struct bt532_ts_platform_data *pdata)
{
	...
	if (!gpio_is_valid(pdata->gpio_vdd)){
		tsp_debug_err(true, dev, "Failed to get vdd_en gpio\n");
		pdata->gpio_vdd = 0;
	}else{
