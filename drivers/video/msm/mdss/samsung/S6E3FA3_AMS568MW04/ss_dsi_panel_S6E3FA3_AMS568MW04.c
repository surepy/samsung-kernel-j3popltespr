...
static struct dsi_panel_cmds *mdss_hbm_etc(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	...
	*level_key = PANEL_LEVE1_KEY;
	...
	if (vdd->auto_brightness == 12) /* for HBM pure mode : 600nit */
		...
		pr_info("%s B6_2ND_PARA = 0x%x\n", __func__, B6_2ND_PARA);

		vdd->dtsi_data[ctrl->ndx].hbm_etc_tx_cmds[vdd->panel_revision].cmds[1].payload[2] = B6_2ND_PARA;
...
static char coordinate_data_1[][COORDINATE_DATA_SIZE] = {
	...
	{0xff, 0x00, 0xfd, 0x00, 0xfe, 0x00}, /* Tune_1 */
	{0xff, 0x00, 0xfd, 0x00, 0xfe, 0x00}, /* Tune_2 */
	{0xff, 0x00, 0xfd, 0x00, 0xfe, 0x00}, /* Tune_3 */
	{0xff, 0x00, 0xfd, 0x00, 0xfe, 0x00}, /* Tune_4 */
	{0xff, 0x00, 0xfd, 0x00, 0xfe, 0x00}, /* Tune_5 */
	{0xff, 0x00, 0xfd, 0x00, 0xfe, 0x00}, /* Tune_6 */
	{0xff, 0x00, 0xfd, 0x00, 0xfe, 0x00}, /* Tune_7 */
	{0xff, 0x00, 0xfd, 0x00, 0xfe, 0x00}, /* Tune_8 */
	{0xff, 0x00, 0xfd, 0x00, 0xfe, 0x00}, /* Tune_9 */
};
...
static struct dsi_panel_cmds * mdss_elvss_temperature2(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	...
	if (vdd->temperature > 0) {
		if (candela_value <= 15)	/* 2 ~ 15 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x0D;
		else				/* 16 ~ 420 nit */
...
{
	u64 time_start,time_end, timeuse;
	u32 current_line;
	...
	if(vdd->multires_stat.prev_mode != vdd->multires_stat.curr_mode) {
		...
		current_line = mdp5_data->ctl->ops.read_line_cnt_fnc(mdp5_data->ctl);
		pr_info("current_line = %d\n", current_line);

		while(current_line > 50)
		{
			usleep_range(500, 500);
			current_line = mdp5_data->ctl->ops.read_line_cnt_fnc(mdp5_data->ctl);
			pr_info("one more current_line = %d\n", current_line);
		}
...
static void mdss_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	/* AID Interpolation */
	vdd->aid_subdivision_enable = false;
