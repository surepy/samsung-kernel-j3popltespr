				...
				struct sec_battery_info *battery)
{
	...
	case SEC_BATTERY_TEMP_CHECK_TEMP:
		temp_value = battery->mixed_temperature;
		break;
	default:
		dev_err(battery->dev,
				...
				struct sec_battery_info *battery)
{
	...
	case SEC_BATTERY_THERMAL_SOURCE_ADC:
		...
		if (battery->pdata->discharging_temp_alpha &&
			battery->pdata->charging_temp_alpha) {
			temp_alpha = battery->status == POWER_SUPPLY_STATUS_DISCHARGING ||
					battery->status == POWER_SUPPLY_STATUS_NOT_CHARGING ||
					battery->cable_type == POWER_SUPPLY_TYPE_BATTERY ||
					is_wireless_type(battery->cable_type) ? battery->pdata->discharging_temp_alpha : battery->pdata->charging_temp_alpha;

			battery->mixed_temperature =
					((battery->temperature + battery->chg_temp) / 2) * temp_alpha / 100;

			if(battery->temperature > 300){
				battery->mixed_temperature =
					MAX_VALUE(battery->mixed_temperature,battery->temperature);
			}else{
				battery->mixed_temperature =
					MIN_VALUE(battery->mixed_temperature,battery->temperature);	
			}
		}else{
			battery->mixed_temperature = battery->temperature;
		}
...
#if defined(CONFIG_BATTERY_SWELLING)
	sec_bat_swelling_check(battery, battery->mixed_temperature);
		...
		struct sec_battery_info *battery)
{
	...
	ret = of_property_read_u32(np, "battery,discharging_temp_alpha",
		&pdata->discharging_temp_alpha);
	if (ret) {
		pr_info("%s: discharging_temp_alpha is Empty\n", __func__);
		pdata->discharging_temp_alpha = 0;
	}

	ret = of_property_read_u32(np, "battery,charging_temp_alpha",
		&pdata->charging_temp_alpha);
	if (ret) {
		pr_info("%s: charging_temp_alpha is Empty\n", __func__);
		pdata->charging_temp_alpha = 0;
	}
