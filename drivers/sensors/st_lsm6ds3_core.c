...
int st_lsm6ds3_acc_set_enable(struct lsm6ds3_data *cdata, bool enable)
{
	...
	if(enable){
		for (i = 0; i < ST_LSM6DS3_ODR_LIST_NUM; i++) {
			if (st_lsm6ds3_odr_table.odr_avl[i].hz == cdata->sdata->acc_c_odr)
				break;
		}
		...
		err = st_lsm6ds3_write_data_with_mask(cdata,
				...
				st_lsm6ds3_odr_table.odr_avl[3].value);
		if (err < 0)
			return err;

		cdata->sdata->acc_c_odr = st_lsm6ds3_odr_table.odr_avl[i].hz;
		cdata->sensors_enabled |= (1 << ST_INDIO_DEV_ACCEL);

		hrtimer_start(&cdata->acc_timer, cdata->delay, HRTIMER_MODE_REL);
	} else {
...
int st_lsm6ds3_gyro_set_enable(struct lsm6ds3_data *cdata, bool enable)
{
	...
	if(enable){
		for (i = 0; i < ST_LSM6DS3_ODR_LIST_NUM; i++) {
			if (st_lsm6ds3_odr_table.odr_avl[i].hz == cdata->sdata->gyr_c_odr)
				break;
		}
		if (i == ST_LSM6DS3_ODR_LIST_NUM)
			return -EINVAL;
		...
		cdata->sdata->gyr_c_odr = st_lsm6ds3_odr_table.odr_avl[i].hz;
		cdata->sensors_enabled |= (1 << ST_INDIO_DEV_GYRO);

		hrtimer_start(&cdata->gyro_timer, cdata->gyro_delay, HRTIMER_MODE_REL);
	} else {
		if (cdata->sensors_enabled & ST_LSM6DS3_EXTRA_DEPENDENCY) {
			err = st_lsm6ds3_write_data_with_mask(cdata,
				st_lsm6ds3_odr_table.addr[ST_INDIO_DEV_GYRO],
...
static int st_lsm6ds3_init_sensor(struct lsm6ds3_data *cdata)
{
	...
	st_lsm6ds3_set_fs(cdata, cdata->sdata->acc_c_gain,0);
	st_lsm6ds3_set_fs(cdata, cdata->sdata->gyr_c_gain,1);
...
static int st_lsm6ds3_set_selftest(struct lsm6ds3_data *cdata, int index, int sindex)
{
	int err;
	u8 mode, mask;
...
static int st_lsm6ds3_gyr_fifo_test(struct lsm6ds3_data *cdata,
...
read_zero_rate_again:
	...
	while (1) {
		...
		if (err < 0) {
			...
			if (buf[1] & 0xA0) {
				...
				if ((--length) == 0) {
					dev_err(cdata->dev, "FIFO not filled.\n");
					return -EBUSY;
				}
			}
		}
	...
	for (i = 0; i < length; i++) {
		...
		if (err < 0) {
			dev_err(cdata->dev, "Reading FIFO output is fail.\n");
			return err;
		}
	}
...
static int st_lsm6ds3_gyr_hw_selftest(struct lsm6ds3_data *cdata,
		s32 NOST[3], s32 ST[3], s32 DIFF_ST[3])
{
	int err;
	u8 buf[1] = {0x00,};
...
static int st_lsm6ds3_selftest_run(struct lsm6ds3_data *cdata,	int test, char *out_str, u8 sindex)
{
	int err;
	...
	pr_info("%s, start\n", __func__);
	/* check ID */
	err = st_lsm6ds3_reg_read(cdata, ST_LSM6DS3_WAI_ADDRESS, 1, &init_status);
	if (err < 0) {
		dev_err(cdata->dev, "failed to read Who-Am-I register.\n");
		return err;
	}
	if (init_status != ST_LSM6DS3_WAI_EXP) {
		dev_err(cdata->dev, "Who-Am-I register value is wrong %x.\n", init_status);
		init_status = 0;
	}
	else {
		init_status = 1;
	...
	err = st_lsm6ds3_reg_read(cdata, ST_LSM6DS3_CTRL1_ADDR, 10, backup_regs2);
	if (err < 0) {
		dev_err(cdata->dev, "failed to read ctrl registers.\n");
		goto restore_md;
	}
	err = st_lsm6ds3_reg_read(cdata, ST_LSM6DS3_FIFO_CTRL1_ADDR, 5, backup_regs1);
	if (err < 0) {
		dev_err(cdata->dev, "failed to read fifo registers.\n");
		goto restore_ctrl;
	}
	...
	if (err < 0) {
		dev_err(cdata->dev, "failed to write 0 into MD registers.\n");
		goto restore_regs;
	}
	err = st_lsm6ds3_reg_write(cdata, ST_LSM6DS3_INT1_ADDR, 2, zero);
	if (err < 0) {
		dev_err(cdata->dev, "failed to write 0 into INT registers.\n");
		goto restore_regs;
	}
	err = st_lsm6ds3_reg_write(cdata, ST_LSM6DS3_FIFO_CTRL1_ADDR, 5, zero);
	if (err < 0) {
		dev_err(cdata->dev, "failed to write 0 into CTRL registers.\n");
		goto restore_regs;
	}
	zero[2] = 0x04;
	zero[8] = 0x38;
	...
	case ST_INDIO_DEV_ACCEL:
		if (test & 2) {
			hw_st_ret = st_lsm6ds3_acc_hw_selftest(cdata, NOST, ST, DIFF_ST);
			if (hw_st_ret > 0) {
	...
	case ST_INDIO_DEV_GYRO:
		if (test & 1) {
			err = st_lsm6ds3_gyr_fifo_test(cdata, zero_rate_data,
				&fifo_count, &fifo_pass, slot_raw, out_str);
			if (err < 0) {
				dev_err(cdata->dev, "Gyro FIFO test fail (%d).\n", fifo_count);
				pr_info("%s, fail raw = %d,%d,%d\n", __func__, slot_raw[0], slot_raw[1], slot_raw[2]);
				goto restore_regs;
			}
		}
		if (test & 2) {
			hw_st_ret = st_lsm6ds3_gyr_hw_selftest(cdata, NOST, ST, DIFF_ST);
			cal_pass = st_lsm6ds3_gyro_save_calibration(cdata, NOST);
...
restore_regs:
	...
	if (err < 0) {
		dev_err(cdata->dev, "failed to write fifo registers.\n");
	}
restore_ctrl:
	err = cdata->tf->write(&cdata->tb, cdata->dev,
					ST_LSM6DS3_CTRL1_ADDR, 10, backup_regs2);
	if (err < 0) {
		dev_err(cdata->dev, "failed to write ctrl registers.\n");
	}
restore_md:
	err = cdata->tf->write(&cdata->tb, cdata->dev,
					ST_LSM6DS3_MD1_ADDR, 2, backup_regs3);
	if (err < 0) {
		dev_err(cdata->dev, "failed to write md registers.\n");
	}
restore_int:
	err = cdata->tf->write(&cdata->tb, cdata->dev,
					ST_LSM6DS3_INT1_ADDR, 2, backup_regs4);
	if (err < 0) {
		dev_err(cdata->dev, "failed to write int registers.\n");
	}
restore_exit:
//	st_lsm6ds3_reg_dump(sdata);
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = st_lsm6ds3_selftest_run(cdata, cdata->sdata->st_mode, buf, ST_INDIO_DEV_GYRO);
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = st_lsm6ds3_selftest_run(cdata, cdata->sdata->st_mode, buf, ST_INDIO_DEV_ACCEL);
...
static void st_lsm6ds3_irq_management(struct work_struct *data_work)
{
	...
	SENSOR_INFO("src_value_reg1= %x\n", src_value_reg1);

	err = cdata->tf->read(&cdata->tb, cdata->dev,
				ST_LSM6DS3_SRC_FUNC_REG, 1, &src_value_reg2);
	if (err < 0)
		goto st_lsm6ds3_irq_enable_irq;
	SENSOR_INFO("src_value_reg2= %x\n", src_value_reg2);

	err = cdata->tf->read(&cdata->tb, cdata->dev,
        	ST_LSM6DS3_SRC_WAKE_UP_REG, 1, &src_wake_up);
	if (err < 0)
        	goto st_lsm6ds3_irq_enable_irq;
	SENSOR_INFO("src_wake_up= %x\n", src_wake_up);
	...
	if ((src_value_reg2 & ST_LSM6DS3_SRC_SIGN_MOTION_DATA_AVL)||(src_value_reg2 & ST_LSM6DS3_SRC_STEP_COUNTER_DATA_AVL)) {
		SENSOR_INFO("########### SMD ############\n");
				...
				struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if ((enable == 1) && (cdata->sa_flag == 0)) {
		...
		if (factory_mode == 1) {
			...
			for (i = 0; i < ST_LSM6DS3_FS_LIST_NUM; i++) {
				if (st_lsm6ds3_fs_table[ST_INDIO_DEV_ACCEL].fs_avl[i].gain == cdata->sdata->acc_c_gain)
					break;
			}
		...
		if (!(cdata->sensors_enabled & (1 << ST_INDIO_DEV_ACCEL))){
			st_lsm6ds3_set_axis_enable(cdata, ST_LSM6DS3_ENABLE_AXIS, ST_INDIO_DEV_ACCEL);
		}
		st_lsm6ds3_reg_read(cdata, ST_LSM6DS3_WU_SRC_ADDR, 1, &val);
		SENSOR_INFO("WU_SRC= %x\n", val);
		...
		cdata->sa_flag = 0;
		if (!(cdata->sensors_enabled & (1 << ST_INDIO_DEV_ACCEL))){
			st_lsm6ds3_set_axis_enable(cdata, 0x00, ST_INDIO_DEV_ACCEL);
		}
		st_lsm6ds3_reg_read(cdata, ST_LSM6DS3_WU_SRC_ADDR, 1, &val);
...
static int st_lsm6ds3_enable_step_c(struct lsm6ds3_data *cdata,
								bool enable)
{
	int err;
...
{
	struct lsm6ds3_data *client_data = container_of(timer,
					struct lsm6ds3_data, gyro_timer);

	if (!work_pending(&client_data->gyro_work))
		queue_work(client_data->gyro_wq, &client_data->gyro_work);

	hrtimer_forward_now(&client_data->gyro_timer, client_data->gyro_delay);
...
static enum hrtimer_restart acc_timer_func(struct hrtimer *timer)
{
	...
	if (!work_pending(&client_data->acc_work))
		queue_work(client_data->accel_wq, &client_data->acc_work);

	hrtimer_forward_now(&client_data->acc_timer, client_data->delay);
...
{
	struct lsm6ds3_data *client_data =
	...
	len = client_data->tf->read(&client_data->tb, client_data->dev,
	...
	tmp_data[0] = SENSOR_X_DATA(raw_data[0], raw_data[1], raw_data[2], \
                              client_data->orientation[0],client_data->orientation[1],client_data->orientation[2],\
                              client_data->orientation[3],client_data->orientation[4],client_data->orientation[5], \
                              client_data->orientation[6],client_data->orientation[7],client_data->orientation[8]);
	...
	client_data->accel_data[0] = tmp_data[0] - client_data->accel_cal_data[0];
	client_data->accel_data[1] = tmp_data[1] - client_data->accel_cal_data[1];
	client_data->accel_data[2] = tmp_data[2] - client_data->accel_cal_data[2];

	time_hi = (int)((timestamp_new & TIME_HI_MASK) >> TIME_HI_SHIFT);
	time_lo = (int)(timestamp_new & TIME_LO_MASK);

	input_report_rel(client_data->acc_input, REL_X, client_data->accel_data[0]);
	input_report_rel(client_data->acc_input, REL_Y, client_data->accel_data[1]);
	input_report_rel(client_data->acc_input, REL_Z, client_data->accel_data[2]);
...
exit:
	if ((ktime_to_ns(client_data->delay) * client_data->time_count)
			>= ((int64_t)ACCEL_LOG_TIME * NSEC_PER_SEC)) {
		SENSOR_INFO("x = %d, y = %d, z = %d\n",
			client_data->accel_data[0], client_data->accel_data[1], client_data->accel_data[2]);
...
static void st_lsm6ds3_gyro_work_func(struct work_struct *work)
{
	...
	client_data->gyro_data[0] = tmp_data[0];
	client_data->gyro_data[1] = tmp_data[1];
	client_data->gyro_data[2] = tmp_data[2];

	time_hi = (int)((timestamp_new & TIME_HI_MASK) >> TIME_HI_SHIFT);
	time_lo = (int)(timestamp_new & TIME_LO_MASK);

	input_report_rel(client_data->gyro_input, REL_RX, client_data->gyro_data[0]);
	input_report_rel(client_data->gyro_input, REL_RY, client_data->gyro_data[1]);
	input_report_rel(client_data->gyro_input, REL_RZ, client_data->gyro_data[2]);
...
exit:
	if ((ktime_to_ns(client_data->gyro_delay) * client_data->gyro_time_count)
			>= ((int64_t)ACCEL_LOG_TIME * NSEC_PER_SEC)) {
		SENSOR_INFO("x = %d, y = %d, z = %d\n",
			client_data->gyro_data[0], client_data->gyro_data[1], client_data->gyro_data[2]);
		...
		struct device_attribute *attr, char *buf)
{
	...
	struct lsm6ds3_data *client_data = input_get_drvdata(input);

	return snprintf(buf, 16, "%lld\n", ktime_to_ns(client_data->delay));
}
		...
		const char *buf, size_t count)
{
	...
	client_data->delay = ns_to_ktime(data);
		...
		const char *buf, size_t count)
{
	...
	if (enable) {
		if (pre_enable == 0) {
			st_lsm6ds3_acc_open_calibration(client_data);
			st_lsm6ds3_set_fs(client_data, client_data->sdata->acc_c_gain,0);
			st_lsm6ds3_acc_set_enable(client_data, true);
			st_lsm6ds3_set_axis_enable(client_data, ST_LSM6DS3_ENABLE_AXIS, ST_INDIO_DEV_ACCEL);
			atomic_set(&client_data->wkqueue_en, 1);
		}
	} else {
		if (pre_enable == 1) {
			if (client_data->sa_flag)
				client_data->sensors_enabled &= ~(1 << ST_INDIO_DEV_ACCEL);
			else {
				atomic_set(&client_data->wkqueue_en, 0);
				st_lsm6ds3_set_axis_enable(client_data, 0x00, ST_INDIO_DEV_ACCEL);
				st_lsm6ds3_acc_set_enable(client_data, false);
			}
		}
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, 16, "%lld\n", ktime_to_ns(client_data->gyro_delay));
}
		...
		const char *buf, size_t count)
{
	...
	client_data->gyro_delay= ns_to_ktime(data);
		...
		const char *buf, size_t count)
{
	...
	if (enable) {
		if (pre_enable == 0) {
			st_lsm6ds3_gyro_open_calibration(client_data);
			st_lsm6ds3_set_fs(client_data, client_data->sdata->gyr_c_gain,1);
			st_lsm6ds3_gyro_set_enable(client_data, true);
			st_lsm6ds3_set_axis_enable(client_data,ST_LSM6DS3_ENABLE_AXIS, ST_INDIO_DEV_GYRO);
			atomic_set(&client_data->gyro_wkqueue_en, 1);
		}
	} else {
		if (pre_enable == 1) {
			atomic_set(&client_data->gyro_wkqueue_en, 0);
			st_lsm6ds3_set_axis_enable(client_data, 0x00, ST_INDIO_DEV_GYRO);
			st_lsm6ds3_gyro_set_enable(client_data, false);
		}
	}
	mutex_unlock(&client_data->mutex_enable);
		...
		const char *buf, size_t count)
{
	...
	mutex_lock(&client_data->mutex_enable);
	if (enable) {
		err = st_lsm6ds3_enable_step_c(client_data, true);
		if (err < 0)
			return err;
		err = st_lsm6ds3_write_data_with_mask(client_data,
					ST_LSM6DS3_SIGN_MOTION_EN_ADDR,
					ST_LSM6DS3_SIGN_MOTION_EN_MASK,
					ST_LSM6DS3_EN_BIT);
		if (err < 0)
			return err;
		st_lsm6ds3_set_irq(client_data, 1);
		client_data->sensors_enabled |= (1 << ST_INDIO_DEV_SIGN_MOTION);
	} else {
		st_lsm6ds3_set_irq(client_data, 0);
		err = st_lsm6ds3_write_data_with_mask(client_data,
					ST_LSM6DS3_SIGN_MOTION_EN_ADDR,
					ST_LSM6DS3_SIGN_MOTION_EN_MASK,
					ST_LSM6DS3_DIS_BIT);
		...
		err = st_lsm6ds3_enable_step_c(client_data, false);
		if (err < 0)
			return err;
		client_data->sensors_enabled &= ~(1 << ST_INDIO_DEV_SIGN_MOTION);
	}
		...
		const char *buf, size_t count)
{
	...
	mutex_lock(&client_data->mutex_enable);
	if (enable) {
		err = st_lsm6ds3_set_extra_dependency(client_data, true);
		if (err < 0)
			return err;
		...
		st_lsm6ds3_set_irq(client_data, 1);
		client_data->sensors_enabled |= (1 << ST_INDIO_DEV_TILT);
	} else {
		st_lsm6ds3_set_irq(client_data, 0);
		err = st_lsm6ds3_write_data_with_mask(client_data,
							ST_LSM6DS3_TILT_EN_ADDR,
		...
		err = st_lsm6ds3_set_extra_dependency(client_data, false);
		if (err < 0)
			return err;
...
static int st_lsm6ds3_set_lpf(struct lsm6ds3_data *cdata, int onoff)
{
	...
	pr_info("%s cdata->odr = %x onoff = %d\n", __func__, cdata->sdata->acc_c_odr, onoff);

	for (i = 0; i < ST_LSM6DS3_ODR_LIST_NUM; i++) {
		if(cdata->sdata->acc_c_odr == st_lsm6ds3_odr_table.odr_avl[i].hz) {
...
{
	int ret = 0;
	struct lsm6ds3_data *client_data = dev_get_drvdata(dev);

	if (client_data->lpf_on)
		ret = 1;
	else
		ret = 0;
...
static int st_lsm6ds3_acc_input_init(struct lsm6ds3_data *cdata)
{
	...
	/* sysfs node creation */
	ret = sysfs_create_group(&dev->dev.kobj, &st_lsm6ds3_accel_attribute_group);
	if (ret < 0)
...
static int st_lsm6ds3_gyro_input_init(struct lsm6ds3_data *cdata)
{
	...
	/* sysfs node creation */
	ret = sysfs_create_group(&dev->dev.kobj, &st_lsm6ds3_gyro_attribute_group);
	if (ret < 0)
		goto err_create_sysfs_group;
...
static int st_lsm6ds3_smd_input_init(struct lsm6ds3_data *cdata)
{
	...
	/* sysfs node creation */
	ret = sysfs_create_group(&dev->dev.kobj, &st_lsm6ds3_smd_attribute_group);
	if (ret < 0)
		goto err_create_sysfs_group;
...
static int st_lsm6ds3_tilt_input_init(struct lsm6ds3_data *cdata)
{
	...
	/* sysfs node creation */
	ret = sysfs_create_group(&dev->dev.kobj, &st_lsm6ds3_tilt_attribute_group);
	if (ret < 0)
		goto err_create_sysfs_group;
...
int st_lsm6ds3_common_probe(struct lsm6ds3_data *cdata, int irq)
{
	...
	for (i = 0; i < ST_INDIO_DEV_NUM; i++) {
		if (i == ST_INDIO_DEV_ACCEL) {
			cdata->sdata->acc_c_odr = st_lsm6ds3_odr_table.odr_avl[3].hz;
			cdata->sdata->acc_c_gain = st_lsm6ds3_fs_table[i].fs_avl[1].gain;
		}
		if (i == ST_INDIO_DEV_GYRO) {
			cdata->sdata->gyr_c_odr = st_lsm6ds3_odr_table.odr_avl[3].hz;
			cdata->sdata->gyr_c_gain = st_lsm6ds3_fs_table[i].fs_avl[2].gain;
		}
	}
...
//	st_lsm6ds3_vdd_on(cdata, 1);
	...
	err = sensors_register(cdata->smd_factory_dev, cdata, smd_sensor_attrs,
		MODULE_NAME_SMD);
	if (err) {
		SENSOR_ERR("failed to sensors_register (%d)\n", err);
		goto err_smd_sensor_register_failed;
	}

	err = sensors_register(cdata->tilt_factory_dev, cdata, tilt_sensor_attrs,
		MODULE_NAME_TILT);
	if (err) {
		SENSOR_ERR("failed to sensors_register (%d)\n", err);
		goto err_smd_sensor_register_failed;
...
err_smd_sensor_register_failed:
...
exit_init_sensor:
exit_of_node:
	sensors_remove_symlink(&cdata->tilt_input->dev.kobj, cdata->tilt_input->name);
	sysfs_remove_group(&cdata->tilt_input->dev.kobj, &st_lsm6ds3_tilt_attribute_group);
	input_unregister_device(cdata->tilt_input);
	input_free_device(cdata->tilt_input);
exit_tilt_input_init:
	sensors_remove_symlink(&cdata->smd_input->dev.kobj, cdata->smd_input->name);
	sysfs_remove_group(&cdata->smd_input->dev.kobj, &st_lsm6ds3_smd_attribute_group);
	input_unregister_device(cdata->smd_input);
	input_free_device(cdata->smd_input);
exit_smd_input_init:
	sensors_remove_symlink(&cdata->gyro_input->dev.kobj, cdata->gyro_input->name);
	sysfs_remove_group(&cdata->gyro_input->dev.kobj, &st_lsm6ds3_gyro_attribute_group);
	input_unregister_device(cdata->gyro_input);
	input_free_device(cdata->gyro_input);
exit_gyro_input_init:
	sensors_remove_symlink(&cdata->acc_input->dev.kobj, cdata->acc_input->name);
	sysfs_remove_group(&cdata->acc_input->dev.kobj, &st_lsm6ds3_accel_attribute_group);
	input_unregister_device(cdata->acc_input);
	input_free_device(cdata->acc_input);
exit_acc_input_init:
	kfree(sdata);
exit:
	return err;
