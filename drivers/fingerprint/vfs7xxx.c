	...
	struct vfsspi_device_data *data)
{
	...
	data->p = pinctrl_get_select(dev, "vfsspi_default");
	if (IS_ERR(data->p)) {
		errorno = -EINVAL;
	...
	data->pins_sleep = pinctrl_lookup_state(data->p, "vfsspi_sleep");
	...
	data->pins_idle = pinctrl_lookup_state(data->p, "vfsspi_idle");
