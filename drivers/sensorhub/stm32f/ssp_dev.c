	...
	struct ssp_data *data)
{
	...
	pr_info("[SSP] rst=%d\n", data->rst);
	if (data->rst < 0) {
		pr_err("[SSP]: GPIO value not correct rst\n");
		goto dt_exit;
	}
	...
	pr_info("[SSP] boot0=%d\n", data->boot0);
	if (data->boot0 < 0) {
		pr_err("[SSP]: GPIO value not correct boot0\n");
		goto dt_exit;
	}
	...
	data->is_mcu_ldo_rev = of_property_read_bool(np, "ssp-ldo-mcu");
	pr_info("[SSP] mcu ldo rev = %d\n",data->is_mcu_ldo_rev);
...
static int ssp_probe(struct spi_device *spi)
{
    ...
    t = kthread_create(ssp_later_init, spi, "ssp_later_init");

    if (IS_ERR(t))  {
        pr_info("%s: failed to create kernel_thread (%ld)!\n",
                __func__, PTR_ERR(t));
        return PTR_ERR(t);
    }
    wake_up_process(t);
...
static int __init ssp_init(void)
{
	...
	ssp_infof("spi_register_driver ret = %d",ret);
	return ret;
