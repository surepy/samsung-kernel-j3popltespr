	...
	case SIOP_LEVEL:
		if (sscanf(buf, "%d\n", &x) == 1) {
			...
			if (x == battery->siop_level && battery->capacity > 5) {
				if (battery->siop_not_skip) {
					dev_info(battery->dev,
							"%s: not skip same siop level: %d\n", __func__, x);
				} else {
					dev_info(battery->dev,
		...
		struct sec_battery_info *battery)
{
	...
	if (ret) {
		pr_info("%s: swelling_low_temp_additional_current is Empty, Defualt value 300mA \n", __func__);
		pdata->swelling_low_temp_additional_current = 300;
	}
